...
def make_colorwheel(transitions: tuple=DEFAULT_TRANSITIONS) -> np.ndarray:
    ...
    start_index = 0
    for hue_to, end_index in zip(base_hues, accumulate(transitions), strict=False):
...
def flow_to_rgb(
    flow: np.ndarray,
    flow_max_radius: float | None=None,
    ...
    def print(self, key=None):
        if key is None:
            for k, v in self.timers.items():
                print(f"Average time for {k}: {v.avg()}")
        else:
            print(f"Average time for {key}: {self.timers[key].avg()}")
...
def solver(
    ...
    print(f"xmin: {xmin_int}, xmax: {xmax_int}, ymin: {ymin_int}, ymax: {ymax_int}, zmin: {zmin_int}, zmax: {zmax_int}")
